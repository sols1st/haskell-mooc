...
firstHalf xs = if odd len then take (div len 2 + 1) xs else take (div len 2) xs where len = length xs
...
capitalize xs = unwords $ map (\(a : as) -> toUpper a : as) $ words xs
...
powers :: Int -> Int -> [Int]
-- powers k max = powers' 0
--   where
--     powers' n = if k ^ n <= max then (k ^ n) : powers' (n + 1) else []
...
(+|+) :: [a] -> [a] -> [a]
-- (+|+) a b = f a ++ f b where f b = [head b | not (null b)]
...
-- sumRights xs = sum [fromRight 0 x |x <- xs,isRight x]
sumRights = sum . map (fromRight 0)

-- sumRights xs = sum $ map (fromRight 0) $ filter isRight xs
...
interpreter = interpreter' 0 0
  where
    interpreter' _ _ [] = []
    interpreter' x y (a : xs) = case a of
      "up" -> interpreter' x (y + 1) xs
      "down" -> interpreter' x (y - 1) xs
      "left" -> interpreter' (x - 1) y xs
      "right" -> interpreter' (x + 1) y xs
      "printX" -> show x : interpreter' x y xs
      "printY" -> show y : interpreter' x y xs
      _ -> interpreter' x y xs