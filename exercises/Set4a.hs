...
rangeOf xs = let s = sort xs in last s - head s
...
longest [x] = x
longest (x : xs)
  | length x > length xx = x
  | length x == length xx = if head x > head xx then xx else x
  | otherwise = xx
  where
    xx = longest xs
...
freqs = foldr (Map.alter func) Map.empty
  where
    func (Just a) = Just (a + 1)
    func Nothing = Just 1
...
transfer from to amount bank
  ...
  | otherwise = Map.insert from (fromJust (Map.lookup from bank) - amount) (Map.insert to (fromJust (Map.lookup to bank) + amount) bank)
