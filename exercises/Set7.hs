...
data State = Start | Error | Finished | FlourAdded | SugarAdded | FlourSugarAdded | EggsAdded | Mixed
  deriving (Eq, Show)
...
reverseNonEmpty (a :| []) = a :| []
reverseNonEmpty (a :| b) = head rb :| tail rb where rb = reverse (a : b)
...
passwordAllowed password (ContainsSome str) = any (`elem` str) password
passwordAllowed password (DoesNotContain str) = not $ any (`elem` str) password
